...
describe("Comment Routes", () => {
  ...
  beforeAll(async () => {
    ...
    const user = await User.create({
      username: `testuser${uuidString}`,
      email: `test${uuidString}@example.com`,
  ...
  afterAll(async () => {
    ...
    if (mongoose.connection) {
      await mongoose.connection.dropCollection("test");
    }
  ...
  describe("GET /comments", () => {
    it("should get all comments", async () => {
      ...
      expect(res.body[0].sender.username).toEqual(`testuser${uuidString}`);
    });
  });
